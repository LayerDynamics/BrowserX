...
export class V8Heap {
  ...
  private objects: Map<HeapObjectID, HeapObject> = new Map();
  private valueToId: WeakMap<JSObject, HeapObjectID> = new WeakMap();
  ...
  allocate(value: JSValue): HeapObjectID {
    ...
    if (value.type === JSValueType.OBJECT || value.type === JSValueType.FUNCTION) {
      this.valueToId.set(value.value as JSObject, id);
    }
    ...
    if (value.type === JSValueType.OBJECT || value.type === JSValueType.FUNCTION) {
      ...
      for (const propValue of obj.properties.values()) {
        if (
          ...
          const childId = this.valueToId.get(propValue.value as JSObject);
          if (childId !== undefined) {
            this.markObject(childId);
          }
        }
      }
      ...
      // Mark prototype
      if (obj.prototype) {
        const protoId = this.valueToId.get(obj.prototype);
        if (protoId !== undefined) {
          this.markObject(protoId);
        }
      }

      // Mark constructor
      if (obj.constructor) {
        const ctorId = this.valueToId.get(obj.constructor as unknown as JSObject);
        if (ctorId !== undefined) {
          this.markObject(ctorId);
        }
      }

      // Mark closure scope (for functions)
      if (value.type === JSValueType.FUNCTION) {
        const fn = value.value as JSFunction;
        if (fn.scope) {
          this.markEnvironment(fn.scope);
        }
      }
    }
  }
  ...
  /**
   * Mark objects referenced from an environment (closure scope chain)
   */
  private markEnvironment(env: Environment): void {
    for (const binding of env.bindings.values()) {
      if (binding.type === JSValueType.OBJECT || binding.type === JSValueType.FUNCTION) {
        const id = this.valueToId.get(binding.value as JSObject);
        if (id !== undefined) {
          this.markObject(id);
        }
      }
    ...
    if (env.outer) {
      this.markEnvironment(env.outer);
    }
  }
