...
export class V8Context {
   ...
   */
  private jsValueToNative(value: JSValue, seen?: WeakSet<object>): unknown {
    switch (value.type) {
      case "undefined":
        return undefined;
      case "null":
      ...
      case "object": {
        const obj = value.value as import("./JSValue.ts").JSObject;
        if (!seen) seen = new WeakSet();
        if (seen.has(obj)) return "[Circular]";
        seen.add(obj);
        // Check if array (has numeric keys + length)
        const lengthProp = obj.properties.get("length");
        if (lengthProp && lengthProp.type === "number") {
          const arr: unknown[] = [];
          for (let i = 0; i < (lengthProp.value as number); i++) {
            const elem = obj.properties.get(String(i));
            arr.push(elem ? this.jsValueToNative(elem, seen) : undefined);
          }
          return arr;
        }
        const result: Record<string, unknown> = {};
        for (const [key, propValue] of obj.properties) {
          if (typeof key === "string") {
            result[key] = this.jsValueToNative(propValue, seen);
          }
        }
        return result;
      }
      case "function": {
        const fn = value.value as import("./JSValue.ts").JSFunction;
