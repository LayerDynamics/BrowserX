...
import { ComputePipelineManager, type PipelineResult } from "../../pipelines/mod.ts";
...
export class ComputePipeline {
  ...
  // Pipeline cache
  private readonly pipelines: Map<string, PipelineResult<GPUComputePipeline>> = new Map();
   ...
   */
  async createPipeline(config: ComputeConfig): Promise<PipelineResult<GPUComputePipeline>> {
    ...
    // Create new pipeline via FFI-first async compilation
    const result = await this.pipelineManager.createSimplePipeline(
      config.shader,
   ...
   */
  getPipeline(config: ComputeConfig): PipelineResult<GPUComputePipeline> | null {
    const key = this.getPipelineKey(config);
  ...
  async executeComputePass(
    ...
    this.stats.totalWorkgroups += config.dispatchWorkgroups.x *
      config.dispatchWorkgroups.y *
      config.dispatchWorkgroups.z;
  ...
  getStatistics(): ComputeStatistics {
    const avgDispatchTime = this.stats.dispatchTimes.length > 0
