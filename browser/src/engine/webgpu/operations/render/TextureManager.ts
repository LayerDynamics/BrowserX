 ...
 */
// Blit/downsample shader for mipmap generation (fullscreen triangle)
const BLIT_SHADER_CODE = `
struct VertexOutput {
...
export class WebGPUTextureManager {
  ...
  /**
   * Initialize the blit pipeline for mipmap downsampling.
   * Lazy-initialized on first call to generateMipmaps().
   */
  private initBlitPipeline(format: GPUTextureFormat): void {
    if (this.blitPipeline) return;

    const gpuDevice = this.device.getDevice();

    this.blitBindGroupLayout = gpuDevice.createBindGroupLayout({
    ...
    this.blitSampler = gpuDevice.createSampler({
      label: "blit-linear-sampler",
      magFilter: "linear",
      minFilter: "linear",
    });
    ...
    const shaderModule = gpuDevice.createShaderModule({
      label: "blit-shader",
      code: BLIT_SHADER_CODE,
    });

    this.blitPipeline = gpuDevice.createRenderPipeline({
      label: "blit-pipeline",
  ...
  /**
   * Generate mipmaps for texture using a blit/downsample pipeline.
   *
   * Each mip level is generated by rendering the previous level through a
   * fullscreen triangle with a linear sampler, producing a downsampled result.
  ...
  generateMipmaps(textureId: GPUTextureID): void {
    ...
    // Lazy-init the blit pipeline for this texture's format
    this.initBlitPipeline(descriptor.format);
    ...
    for (let mipLevel = 1; mipLevel < mipLevelCount; mipLevel++) {
      ...
      const _mipWidth = mipSize.width;
      const _mipHeight = mipSize.height;
      ...
      // Create bind group binding source texture + linear sampler
      const bindGroup = this.device.getDevice().createBindGroup({
      ...
      // Render pass to downsample: draw fullscreen triangle through linear sampler
      ...
      passEncoder.draw(3); // Fullscreen triangle (3 vertices, no vertex buffer)
