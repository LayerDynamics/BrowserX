...
import type { ByteCount, GPUBufferID, GPUSize, Timestamp } from "../../../types/webgpu.ts";
import { GPUBufferError, GPUBufferMapError, GPUBufferUsageError } from "../errors.ts";
...
export class WebGPUBuffer {
  ...
  write(data: BufferSource | SharedArrayBuffer, offset: number = 0): void {
    ...
    // Validate size
    const dataSize = ArrayBuffer.isView(data) ? data.byteLength : (data as ArrayBuffer).byteLength;
    ...
    // Perform write - wrap as Uint8Array to avoid Deno WebGPU bug where
    // TypedArray size parameter is multiplied by bytes_per_element (queue.rs:185).
    // Using Uint8Array ensures bytes_per_element=1 so dataSize stays in bytes.
    const queue = this.device.getQueue();
    let writeData: Uint8Array;
    if (data instanceof SharedArrayBuffer) {
      const copy = new ArrayBuffer(data.byteLength);
      new Uint8Array(copy).set(new Uint8Array(data));
      writeData = new Uint8Array(copy);
    } else if (ArrayBuffer.isView(data)) {
      // Copy to new ArrayBuffer to satisfy Deno's type requirements
      const copy = new ArrayBuffer(data.byteLength);
    ...
    queue.writeBuffer(
      this.buffer!,
      offset,
      writeData as unknown as Uint8Array<ArrayBuffer>,
      0,
      dataSize,
    );
  ...
  async mapAsync(
    ...
    try {
      ...
      const nativeMode = mode === BufferMapMode.READ ? GPUMapMode.READ : GPUMapMode.WRITE;
