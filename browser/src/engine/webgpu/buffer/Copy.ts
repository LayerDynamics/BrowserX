...
import { GPUBufferError, GPUBufferStateError, GPUBufferUsageError } from "../errors.ts";
...
function validateCopy(descriptor: BufferCopyDescriptor): void {
  ...
  if (sourceOffset + size > sourceSize) {
    throw new GPUBufferError(
      `Source copy range [${sourceOffset}, ${
        sourceOffset + size
      }) exceeds buffer size ${sourceSize}`,
      {
  ...
  if (destinationOffset + size > destSize) {
    throw new GPUBufferError(
      `Destination copy range [${destinationOffset}, ${
        destinationOffset + size
      }) exceeds buffer size ${destSize}`,
      {
        code: "GPU_BUFFER_COPY_OUT_OF_BOUNDS",
  ...
  if (source === destination) {
    ...
    const overlaps = (sourceOffset >= destinationOffset && sourceOffset < destEnd) ||
