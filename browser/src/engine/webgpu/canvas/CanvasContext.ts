...
export class WebGPUCanvasContext {
  ...
  private isHTMLCanvas(canvas: any): boolean {
    return canvas && "getBoundingClientRect" in canvas &&
  ...
  private updateDimensions(): void {
    ...
    const canvasObj = this.canvas as unknown as Record<string, unknown>;
    if (
      "getBoundingClientRect" in this.canvas &&
      typeof canvasObj.getBoundingClientRect === "function"
    ) {
      // HTML canvas - use client size
      const rect = (canvasObj.getBoundingClientRect as () => { width: number; height: number })();
      const dpr = (globalThis as unknown as Record<string, unknown>).devicePixelRatio as number ||
        1;
  ...
  private setupResizeObserver(): void {
    const ResizeObserverClass = (globalThis as unknown as Record<string, unknown>).ResizeObserver as
      | (new (
        callback: (entries: unknown[]) => void,
      ) => { observe(target: unknown): void; disconnect(): void })
      | undefined;
  ...
  getCurrentTexture(): GPUTexture {
    ...
    if (this.textureAcquired && this.configuration) {
      ...
      try {
        ...
        console.warn(
          "[CanvasContext] Failed to unconfigure/reconfigure context for cache invalidation:",
          error,
        );
