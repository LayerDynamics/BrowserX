...
import type { GPUTextureID, LayerID, Pixels, Timestamp } from "../../../types/webgpu.ts";
...
import type { BoundingBox, DisplayList } from "../../rendering/paint/DisplayList.ts";
import { type CanvasImageSource, document } from "../../../types/dom.ts";
...
export class WebGPUCompositorLayer {
  ...
  private async uploadTiledTexture(displayList: DisplayList): Promise<void> {
    for (const tile of this.tiles) {
      ...
      const texture = tile.textureId ? this.textureManager.getTexture(tile.textureId) : null;
  ...
  updateConfig(config: Partial<LayerConfig>): void {
    ...
    if (config.y !== undefined) {
      this.config.y = config.y;
    }
    if (config.zIndex !== undefined) {
      this.config.zIndex = config.zIndex;
    }
    ...
    if (config.clipToBounds !== undefined) {
      this.config.clipToBounds = config.clipToBounds;
    }
