...
import type { Duration, GPUTextureID, Pixels, Timestamp } from "../../../types/webgpu.ts";
...
export class WebGPUCompositorThread {
  ...
  private layerIntersectsDamage(layer: LayerDescriptor): boolean {
    ...
    return this.damageRects.some((damage) => this.rectsIntersect(layerRect, damage));
  }
  ...
  private async compositeLayer(
    ...
    const format =
      (this.canvasContext.getConfiguration()?.format ?? "bgra8unorm") as GPUTextureFormat;
  ...
  stopFrameLoop(): void {
    if (this.animationFrameId !== null) {
      const cancelFn = (globalThis as unknown as Record<string, unknown>).cancelAnimationFrame;
  ...
  private scheduleNextFrame(): void {
    if (this.config.enableVSync) {
      const requestFn = (globalThis as unknown as Record<string, unknown>).requestAnimationFrame;
