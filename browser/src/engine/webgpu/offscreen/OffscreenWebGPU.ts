...
import type { Pixels, Timestamp } from "../../../types/webgpu.ts";
...
export class OffscreenWebGPU {
  ...
  private createReadbackBuffer(): void {
    ...
    if (this.debug) {
      this.logger.info(
        `Created readback buffer ${bufferSize} bytes (padded row: ${paddedBytesPerRow})`,
      );
    }
  }
  ...
  getStatistics(): OffscreenWebGPUStatistics {
    return {
      ...
      averageReadbackTime: this.readbackCount > 0 ? this.totalReadbackTime / this.readbackCount : 0,
