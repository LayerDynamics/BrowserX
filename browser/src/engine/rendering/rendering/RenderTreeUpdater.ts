...
export class RenderTreeUpdater {
  ...
  private handleChildRemoved(
    ...
    // Remove from parent (save ref before removeChild clears it)
    const parentRef = renderObject.parent;
    if (parentRef) {
      parentRef.removeChild(renderObject);
      parentRef.markNeedsLayout();
      parentRef.markNeedsPaint();
    }
