...
/**
 * Interface for render objects that provide a baseline measurement
 */
...
/**
 * Type guard: checks if a render object has a getBaseline method
 */
function hasBaseline(obj: unknown): obj is WithBaseline {
  return typeof obj === "object" && obj !== null && "getBaseline" in obj &&
    typeof (obj as WithBaseline).getBaseline === "function";
}
...
export class FlexboxLayout {
  ...
  private positionItemsInLine(
    ...
    // Pre-calculate baseline for the line if needed
    let lineBaseline = 0 as Pixels;
    if (alignItems === AlignItems.BASELINE) {
      for (const item of line.items) {
        const itemAlignSelf = item.renderObject.style.getPropertyValue("align-self") || "auto";
        const effectiveAlign = itemAlignSelf === "auto" ? alignItems : itemAlignSelf as AlignItems;
        if (effectiveAlign === AlignItems.BASELINE) {
          const baseline = this.getItemBaseline(item, isRow);
          lineBaseline = Math.max(lineBaseline, baseline) as Pixels;
        }
      }
    }
    ...
    for (let i = 0; i < line.items.length; i++) {
      ...
      switch (effectiveAlign) {
        ...
        case AlignItems.BASELINE: {
          const itemBaseline = this.getItemBaseline(item, isRow);
          crossPos = (crossOffset + lineBaseline - itemBaseline) as Pixels;
          break;
        }
        default:
          crossPos = crossOffset;
      ...
      const finalX = isRow ? (containerX + mainPos) as Pixels : (containerX + crossPos) as Pixels;
      const finalY = isRow ? (containerY + crossPos) as Pixels : (containerY + mainPos) as Pixels;
  ...
  /**
   * Get the baseline of a flex item for baseline alignment.
   * The baseline is the distance from the item's cross-start edge to its first baseline.
   */
  private getItemBaseline(item: FlexItem, isRow: boolean): Pixels {
    const ro = item.renderObject;

    // Check if the item has an explicit baseline (e.g., from font metrics)
    if (hasBaseline(ro)) {
      return ro.getBaseline();
    }

    // For items with children, use the first child's baseline
    if (ro.children && ro.children.length > 0) {
      const firstChild = ro.children[0] as RenderBox;
      if (hasBaseline(firstChild)) {
        return firstChild.getBaseline();
      }
      // Approximate: use first child's cross size as baseline
      if (firstChild.layout) {
        return (isRow ? firstChild.layout.height : firstChild.layout.width) as Pixels;
      }
    }

    // Fallback: use the item's cross size (bottom edge = baseline)
