...
>>`;
  ...
  async addImage(
    ...
    const isPNG = imageData[0] === 0x89 && imageData[1] === 0x50 && imageData[2] === 0x4E &&
      imageData[3] === 0x47;
...
>>
stream
${new TextDecoder("latin1").decode(compressed)}
endstream`;
    } else {
      // Assume raw RGB data
      imageContent = `<<
...
endstream`;
  ...
  /**
   * Decode PNG file bytes to raw RGB pixel data.
   * Parses PNG chunks, decompresses IDAT data, applies scanline filters,
   * and converts color types to RGB.
   */
  private async decodePNG(pngData: ByteBuffer): Promise<Uint8Array> {
    // Validate PNG signature
    const PNG_SIG = [0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A];
    for (let i = 0; i < 8; i++) {
      if (pngData[i] !== PNG_SIG[i]) {
    ...
    while (offset + 8 <= pngData.length) {
      ...
      if (chunkType === "IHDR") {
        width = (pngData[chunkDataStart] << 24) | (pngData[chunkDataStart + 1] << 16) |
          (pngData[chunkDataStart + 2] << 8) | pngData[chunkDataStart + 3];
        height = (pngData[chunkDataStart + 4] << 24) | (pngData[chunkDataStart + 5] << 16) |
          (pngData[chunkDataStart + 6] << 8) | pngData[chunkDataStart + 7];
        bitDepth = pngData[chunkDataStart + 8];
        colorType = pngData[chunkDataStart + 9];
      } else if (chunkType === "PLTE") {
        palette = pngData.slice(chunkDataStart, chunkDataStart + chunkLen);
      } else if (chunkType === "IDAT") {
        idatChunks.push(pngData.slice(chunkDataStart, chunkDataStart + chunkLen));
      } else if (chunkType === "IEND") {
        break;
      }

      // Next chunk: length(4) + type(4) + data(chunkLen) + CRC(4)
      offset = chunkDataStart + chunkLen + 4;
    }
    ...
    // Decompress with zlib (PNG uses deflate wrapped in zlib)
    const decompressed = await this.decompressZlib(compressedData);
    if (decompressed.length === 0) return new Uint8Array(0);

    // Determine bytes per pixel based on color type
    const bpp = this.bytesPerPixel(colorType, bitDepth);
    const scanlineBytes = width * bpp;

    // Apply PNG scanline filters
    const filtered = this.applyScanlineFilters(decompressed, width, height, bpp, scanlineBytes);

    // Convert to RGB
    return this.convertToRGB(filtered, width, height, colorType, bitDepth, palette);
  }

  /**
   * Decompress zlib data using Web Streams API
   */
  ...
  /**
   * Compress data with zlib deflate for PDF FlateDecode
   */
  ...
  /**
   * Bytes per pixel for a given PNG color type and bit depth
   */
  private bytesPerPixel(colorType: number, bitDepth: number): number {
    const channelBytes = Math.max(1, bitDepth / 8);
    switch (colorType) {
      case 0:
        return channelBytes; // Grayscale
      case 2:
        return channelBytes * 3; // RGB
      case 3:
        return 1; // Indexed (1 byte palette index)
      case 4:
        return channelBytes * 2; // Grayscale + Alpha
      case 6:
        return channelBytes * 4; // RGBA
      default:
  ...
  /**
   * Reconstruct pixel data by reversing PNG scanline filters.
   * Each scanline is: filterByte(1) + pixelData(scanlineBytes)
   */
  private applyScanlineFilters(
    data: Uint8Array,
    _width: number,
    height: number,
    bpp: number,
    ...
    const result = new Uint8Array(height * scanlineBytes);
    const stride = scanlineBytes + 1; // +1 for filter byte

    for (let y = 0; y < height; y++) {
      const filterByte = data[y * stride];
      const srcOffset = y * stride + 1;
      const dstOffset = y * scanlineBytes;
      const priorOffset = (y - 1) * scanlineBytes;

      for (let x = 0; x < scanlineBytes; x++) {
        const raw = data[srcOffset + x];
        const a = x >= bpp ? result[dstOffset + x - bpp] : 0; // left
        const b = y > 0 ? result[priorOffset + x] : 0; // above
        const c = (x >= bpp && y > 0) ? result[priorOffset + x - bpp] : 0; // upper-left
        ...
        switch (filterByte) {
          ...
          case 3: // Average
            ...
            break;
          case 4: // Paeth
            reconstructed = (raw + this.paethPredictor(a, b, c)) & 0xFF;
            break;
          default:
            reconstructed = raw;
        }
  ...
  /**
   * Convert decoded pixel data to RGB (3 bytes per pixel)
   */
  private convertToRGB(
    ...
    switch (colorType) {
      case 0: // Grayscale → R=G=B=gray
        ...
        break;
      case 2: // RGB — already in correct format
        rgbData.set(pixelData.slice(0, totalPixels * 3));
        break;
      case 3: // Indexed — lookup palette
        if (palette) {
          for (let i = 0; i < totalPixels; i++) {
            const idx = pixelData[i] * 3;
            rgbData[i * 3] = palette[idx];
            rgbData[i * 3 + 1] = palette[idx + 1];
            rgbData[i * 3 + 2] = palette[idx + 2];
          }
        }
        break;
      case 4: // Grayscale + Alpha → discard alpha
        ...
        break;
      case 6: // RGBA → take RGB, discard alpha
        for (let i = 0; i < totalPixels; i++) {
...
class PDFContentStream {
  ...
  transform(a: number, b: number, c: number, d: number, e: number, f: number): void {
    this.commands.push(
      `${a.toFixed(3)} ${b.toFixed(3)} ${c.toFixed(3)} ${d.toFixed(3)} ${e.toFixed(2)} ${
        f.toFixed(2)
      } cm`,
    );
  }
...
>>`;
  ...
  private getImageDimensions(imageData: Uint8Array): { width: number; height: number } {
    ...
    if (
      ...
      const width = (imageData[16] << 24) | (imageData[17] << 16) | (imageData[18] << 8) |
        imageData[19];
      const height = (imageData[20] << 24) | (imageData[21] << 16) | (imageData[22] << 8) |
        imageData[23];
      return { width, height };
    }
