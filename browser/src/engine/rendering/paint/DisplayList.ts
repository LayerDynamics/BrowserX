...
import type { CanvasImageSource, CanvasRenderingContext2D } from "../../../types/dom.ts";
...
export class DisplayList {
  ...
  /**
   * Register a loaded image for use by DRAW_IMAGE commands.
   * @param src - The image source URL (must match DrawImageCommand.src)
   * @param image - The loaded image (ImageBitmap, HTMLCanvasElement, etc.)
   */
  registerImage(src: string, image: CanvasImageSource): void {
    this.imageCache.set(src, image);
  }

  /**
   * Preload all images referenced by DRAW_IMAGE commands.
   * Fetches images that aren't already in the cache and creates ImageBitmaps.
   * Call this before replay() to ensure DRAW_IMAGE commands can render.
   */
  async preloadImages(): Promise<void> {
    const imageSrcs = new Set<string>();
    for (const command of this.commands) {
      if (command.type === PaintCommandType.DRAW_IMAGE) {
        const imgCmd = command as DrawImageCommand;
        if (!this.imageCache.has(imgCmd.src)) {
          imageSrcs.add(imgCmd.src);
        }
      }
    }
    ...
    for (const src of imageSrcs) {
      loadPromises.push(
        fetch(src)
          ...
          .catch(() => {
            // Image load failed — skip silently, DRAW_IMAGE will be a no-op
          }),
      );
    }
  ...
  private executeCommand(context: CanvasRenderingContext2D, command: AnyPaintCommand): void {
    switch (command.type) {
      ...
      case PaintCommandType.DRAW_IMAGE: {
        const img = this.imageCache.get(command.src);
        if (img) {
