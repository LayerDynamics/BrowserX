...
import type { PaintContext } from "../../../types/rendering.ts";
...
export class PaintLayer {
  ...
  /**
   * Paint this layer
   * Records paint commands into display list via a real PaintContext
   */
  paint(): void {
    if (!this.dirty) {
      return;
    ...
    // Create a recording PaintContext that captures commands into the DisplayList.
    // This bridges the PaintContext interface (used by RenderObject.paint()) with
    // the DisplayList format used for compositing/replay.
    const paintContext = this.createRecordingContext();

    // Paint all render objects in this layer
    for (const renderObject of this.renderObjects) {
      if (renderObject.needsPaint) {
        renderObject.paint(paintContext);
      }
    }
  ...
  /**
   * Create a PaintContext that records commands into this layer's DisplayList
   */
  private createRecordingContext(): PaintContext {
    ...
    return {
      ...
      fillText: (text: string, x: Pixels, y: Pixels, font: string, color: string) => {
        displayList.add({
      ...
      drawImage: (src: string, x: Pixels, y: Pixels, width: Pixels, height: Pixels) => {
        displayList.add({
      ...
      transform: (matrix: import("../../../types/rendering.ts").TransformMatrix) => {
        displayList.add({
