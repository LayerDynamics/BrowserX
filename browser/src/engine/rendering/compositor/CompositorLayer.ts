...
export class CompositorLayer {
  ...
  private createTransformMatrix(): Float32Array {
    ...
    // Compose: Translate → Rotate → Scale with transform origin
    // 2D affine transform lifted into column-major 4x4 matrix
    const cos = Math.cos(t.rotation);
    const sin = Math.sin(t.rotation);

    // Row 0: scaleX * cos, scaleX * sin
    matrix[0] = t.scaleX * cos;
    matrix[1] = t.scaleX * sin;
    matrix[2] = 0;
    matrix[3] = 0;

    // Row 1: -scaleY * sin, scaleY * cos
    matrix[4] = -t.scaleY * sin;
    matrix[5] = t.scaleY * cos;
    matrix[6] = 0;
    matrix[7] = 0;
    ...
    // Row 3: Translation accounting for transform origin
    // Origin-adjusted: translate to origin, apply transform, translate back
