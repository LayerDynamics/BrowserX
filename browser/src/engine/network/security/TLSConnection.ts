...
import { loadSystemCAs, validateCertificate } from "./Certificate.ts";
...
export class TLSConnection {
  ...
  async handshake(): Promise<void> {
    ...
    if (isHelloRetryRequest) {
      ...
      const firstClientHelloHash = new Uint8Array(
        await crypto.subtle.digest(hashAlgorithm, this.handshakeMessages[0]),
      );
      ...
      const newClientHello: TLSHandshakeMessage = {
        ...
        extensions: [
          ...
          { type: "supported_groups", data: ["x25519", "secp256r1", "secp384r1"] },
          {
            ...
            data: [0x0403, 0x0804, 0x0401, 0x0503, 0x0805, 0x0501, 0x0806, 0x0601],
          },
      ...
      const handshakeContext = new Uint8Array(
        await crypto.subtle.digest(hashAlgorithm, concat(...this.handshakeMessages)),
      );
      ...
      const applicationContextHash = new Uint8Array(
        await crypto.subtle.digest(hashAlgorithm, concat(...this.handshakeMessages)),
      );
      ...
      this.sessionKeys = {
        clientWriteKey: await SessionKeysUtil.hkdfExpandLabel(
          appSecrets.clientApplicationTrafficSecret,
          "key",
          emptyContext,
          keyLen,
          this.negotiatedCipherSuite,
        ...
        serverWriteKey: await SessionKeysUtil.hkdfExpandLabel(
          appSecrets.serverApplicationTrafficSecret,
          "key",
          emptyContext,
          keyLen,
          this.negotiatedCipherSuite,
  ...
  private async handshakeTLS12(
    ...
    const groupName = curveId === 0x001d
      ? "x25519"
      : curveId === 0x0017
      ? "secp256r1"
      : curveId === 0x0018
    ...
    const finishedRecord = createTLSRecord(
      TLSRecordType.HANDSHAKE,
      encryptedFinished as ByteBuffer,
    );
    ...
    const decryptedServerFinished = await this.decryptTLS12Record(
      record.data,
      TLSRecordType.HANDSHAKE,
    );
   ...
   */
  private async encryptTLS12Record(
   ...
   */
  private async decryptTLS12Record(
    data: ByteBuffer,
    contentType: TLSRecordType,
  ): Promise<ByteBuffer> {
    if (!this.sessionKeys) throw new TLSError("No session keys");
  ...
  private async computeSharedSecret(
    ...
    if (curvesMatch && clientKeyShare.cryptoKeyPair) {
      try {
        ...
        throw new TLSError(
          `ECDH key exchange failed for ${serverKeyShare.group}: ${(error as Error).message}`,
        );
      }
    }
  ...
  private async createFinished(): Promise<TLSHandshakeMessage> {
    ...
    const verifyData = await SessionKeysUtil.hmacWithCipherSuite(
      finishedKey,
      transcriptHash,
      this.negotiatedCipherSuite,
    );
  ...
  private async receiveServerHello(): Promise<TLSHandshakeMessage> {
    ...
    if (record.type === TLSRecordType.ALERT) {
      ...
      const alertDesc = record.data[1];
      throw new TLSError(
        `Server sent TLS alert: level=${alertLevel}, description=${alertDesc} (${
          this.getAlertDescription(alertDesc)
        })`,
      );
    }
   ...
   */
  private async decryptHandshakeRecord(
    ...
    try {
      ...
      throw new TLSError(
        `TLS 1.3 handshake record decryption failed: ${
          (error as Error).message || "authentication tag verification failed"
        }`,
      );
    }
...
class DERParser {
  ...
  parseExplicitTag(expectedTag: number): ByteBuffer {
    ...
    if (tag !== expectedTagByte) {
      throw new Error(
        `DER parsing error: expected explicit tag [${expectedTag}] (0x${
          expectedTagByte.toString(16)
        }), got 0x${tag.toString(16)}`,
      );
    }
...
function parseHandshakeMessage(data: ByteBuffer): TLSHandshakeMessage {
  ...
  if (data.byteLength < 4 + length) {
    throw new TLSError(
      `Invalid handshake message: expected ${4 + length} bytes, got ${data.byteLength}`,
    );
  }
