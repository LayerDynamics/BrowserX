...
export class HTTP2Connection {
   ...
   *
   * GOAWAY payload:
   *   - Last-Stream-ID (31 bits, offset 0)
   *   - Error Code (32 bits, offset 4)
   *   - Optional debug data (remaining bytes)
   *
   * Streams with ID > lastStreamId were never processed by the server
   * and must be rejected so they can be retried on a new connection.
   */
  private async handleGoawayFrame(frame: HTTP2Frame): Promise<void> {
    ...
    const lastStreamId = view.getUint32(0) & 0x7FFFFFFF;
    const errorCode = view.getUint32(4) as HTTP2ErrorCode;

    if (errorCode !== HTTP2ErrorCode.NO_ERROR) {
      console.warn(
        `GOAWAY: error code ${errorCode}, last stream ${lastStreamId}`,
      );
    }

    // Reject all streams with ID greater than lastStreamId —
    // the server will not process them.
    for (const [streamId, stream] of this.streams) {
      if (streamId > lastStreamId) {
        stream.state = HTTP2StreamState.CLOSED;
        if (stream.responseReject) {
          stream.responseReject(
            new Error(
              `Stream ${streamId} refused by GOAWAY (last accepted: ${lastStreamId})`,
            ),
          );
        }
    ...
    // If no remaining active streams, close the connection
