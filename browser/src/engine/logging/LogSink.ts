...
export class StderrSink implements LogSink {
  ...
  write(entry: LogEntry): void {
    ...
    const method = entry.level === "error" ? "error" : entry.level === "warn" ? "warn" : entry.level === "debug" ? "debug" : "log";
    if (entry.data !== undefined) {
      (console as Record<string, (...a: unknown[]) => void>)[method](prefix, entry.message, entry.data);
    } else {
