...
export class PerformanceProfiler {
  ...
  getNavigationTiming(): NavigationTiming {
    ...
    const navigationTiming: NavigationTiming = {
      ...
      domContentLoadedEventStart: currentTime + timing.htmlFetch + timing.htmlParse +
        timing.cssFetch + timing.cssParse,
      domContentLoadedEventEnd: currentTime + timing.htmlFetch + timing.htmlParse +
        timing.cssFetch + timing.cssParse + timing.scriptExecution,
  ...
  getWebVitals(): WebVitals {
    ...
    const fcp = timing.htmlFetch + timing.htmlParse + timing.styleResolution +
      timing.paintRecording * 0.5;
  ...
  getPaintTiming(): PaintTiming {
    ...
    const baseTime = timing.htmlFetch + timing.htmlParse + timing.cssFetch + timing.cssParse +
  ...
  getSummary(): {
    ...
    if (vitals.lcp && vitals.lcp > 2500) {
      recommendations.push(
        "Optimize Largest Contentful Paint - consider lazy loading images and preloading critical resources",
      );
    }
    ...
    if (vitals.ttfb && vitals.ttfb > 200) {
      recommendations.push(
        "Reduce Time to First Byte - optimize server response time or use a CDN",
      );
    }

    if (metrics.scriptExecutionMs > 500) {
      recommendations.push(
        "Reduce JavaScript execution time - consider code splitting or deferring non-critical scripts",
      );
    }

    if (metrics.layoutComputationMs > 100) {
      recommendations.push(
        "Simplify layout - reduce DOM complexity and avoid forced synchronous layouts",
      );
    }

    if (network.cacheHitRatio < 0.3) {
      recommendations.push(
        "Improve caching - set appropriate cache headers and use service workers",
      );
    }
