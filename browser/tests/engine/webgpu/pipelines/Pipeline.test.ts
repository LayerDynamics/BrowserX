...
if (webgpuAvailable) {
  ...
  async function setup() {
    device = await getSharedDevice();
  }

  function teardown() {
    // Device is shared — do not destroy it here.
  }
  ...
  // ========================================================================
  // Async Pipeline Compilation Tests
  // ========================================================================

  Deno.test("RenderPipelineManager - async pipeline creation with enableAsync", async () => {
    await setup();

    const manager = new RenderPipelineManager(device, { enableAsync: true });
    const gpuDevice = device.getDevice();
  ...
  Deno.test("ComputePipelineManager - async pipeline creation with enableAsync", async () => {
    await setup();

    const manager = new ComputePipelineManager(device, { enableAsync: true });
    const gpuDevice = device.getDevice();
  ...
  Deno.test("RenderPipelineManager - async pipeline caches correctly", async () => {
    await setup();
  ...
  Deno.test("ComputePipelineManager - async pipeline caches correctly", async () => {
    await setup();
