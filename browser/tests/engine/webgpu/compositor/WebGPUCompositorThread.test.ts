...
import type { GPUTextureID, Pixels } from "../../../../src/types/webgpu.ts";
...
async function createTestCompositor(canvasW = 800, canvasH = 600): Promise<{
  ...
  return { compositor, device, canvasCtx };
}
...
if (webgpuAvailable) {
  Deno.test({
    ...
    async fn() {
      const { compositor, device, canvasCtx } = await createTestCompositor();
