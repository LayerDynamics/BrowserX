...
import type { LayerID, Pixels } from "../../../../src/types/webgpu.ts";
...
// Shared texture manager across all tests
let sharedTextureManager: WebGPUTextureManager | null = null;

function getSharedTextureManager(device: WebGPUDevice): WebGPUTextureManager {
  if (!sharedTextureManager) {
    sharedTextureManager = new WebGPUTextureManager(device);
  }
...
if (webgpuAvailable) {
  ...
  Deno.test("CompositorLayer - creates with correct initial state", async () => {
    ...
    const textureManager = getSharedTextureManager(device);
